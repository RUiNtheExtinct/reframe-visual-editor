...
export function buildTsxWithStyleOverrides(
  ...
  // Wrapper name uses a sanitized identifier
  const baseRaw = preferredName?.trim() || innerName || "Component";
  const wrapperBase = sanitizeIdentifier(baseRaw);
  ...
  // If source has a default export, rewrite it to a named export so wrapper can be default
  const transformed = hasDefault
    ? rewriteDefaultExportToNamed(cleanedSource, innerName)
    : cleanedSource;
  return `${transformed}${suffix}`;
}

function sanitizeIdentifier(input: string): string {
  let out = input.replace(/[^A-Za-z0-9_]/g, "");
  if (!out) out = "Component";
  if (!/^[A-Za-z_]/.test(out)) out = `C${out}`;
  return out;
}

function rewriteDefaultExportToNamed(source: string, name: string): string {
  let out = source;
  // export default function Name(...) -> export function Name(...)
  out = out.replace(new RegExp(`(^|\\n)\\s*export\\s+default\\s+function\\s+${name}\\b`), (m) =>
    m.replace("export default ", "export ")
  );
  // export default class Name ... -> export class Name ...
  out = out.replace(new RegExp(`(^|\\n)\\s*export\\s+default\\s+class\\s+${name}\\b`), (m) =>
    m.replace("export default ", "export ")
  );
  // export default Name; -> remove that line
